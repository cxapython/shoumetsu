package com.google.android.gms.internal.drive;

import android.os.IInterface;
import com.google.android.gms.common.api.Status;

/* loaded from: classes.dex */
public interface zzeq extends IInterface {
    void onSuccess();

    void zza(Status status);

    void zza(com.google.android.gms.drive.zza zzaVar);

    void zza(zzem zzemVar);

    void zza(zzez zzezVar);

    void zza(zzfb zzfbVar);

    void zza(zzfd zzfdVar);

    void zza(zzff zzffVar);

    void zza(zzfh zzfhVar);

    void zza(zzfl zzflVar);

    void zza(zzfn zzfnVar);

    void zza(zzfp zzfpVar);

    void zza(zzfr zzfrVar, zzil zzilVar);

    void zza(zzfs zzfsVar);

    void zza(zzfu zzfuVar);

    void zza(zzfx zzfxVar);

    void zza(zzfz zzfzVar);

    void zza(zzgb zzgbVar);

    void zza(zzgt zzgtVar);

    void zzb(boolean z);
}
